...
class ModelTest(TestCase):
    def setUp(self):
        Event.objects.create(id=1, name="oneplue 3 event", status=True,limit =2000,address='shenzhen',
    ...
    def test_event_models(self):
        result = Event.objects.get(name="oneplue 3 event ")
...
class GuestManageTest(TestCase):
    ...
    def test_guest_mange_search_success(self):
        ''' 测试嘉宾搜索 '''
        response = self.client.post('/login_action/',data=self.login_user)
        response = self.client.post('/search_phone/',{"phone":"13000010"})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"alen", response.content)
        self.assertIn(b"13000010", response.content)
...
class SignIndexActionTest(TestCase):
    ...
    def test_002_sign_index_action_phone_or_event_id_error(self):
        ''' 手机号或发布会id错误 '''
        response = self.client.post('/login_action/',data=self.login_user)
        response = self.client.post('/sign_index_action/2/',
                                    {"phone": "18611001100"})
        self.assertEqual(response.status_code, 200)
